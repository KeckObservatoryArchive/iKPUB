...
Usage (MongoDB):
    python src/data/ingest.py --mongo --start-year 2024 --end-year 2025
    python src/data/ingest.py --mongo --collection test_articles --skip-query --start-year 2020 --end-year 2025
"""
...
from data.fetch_full_text import run as fetch_full_text, run_mongo as fetch_full_text_mongo
...
def main():
    ...
    if args.mongo:
        ...
        if not args.skip_query:
            ...
            print("  (ADS query to MongoDB not yet implemented — use --skip-query)")
        else:
            print("\n=== Step 1: Query ADS (skipped) ===")

        if not args.skip_fulltext:
            print(f"\n=== Step 2: Fetch full text (MongoDB) ===")
            for year in range(args.start_year, args.end_year + 1):
                fetch_full_text_mongo(year, args.collection)
        else:
            print("\n=== Step 2: Fetch full text (skipped) ===")

        print("\nDone. Full text saved to data/pubs/full_text/. Run predict_labels.py --mongo to classify.")
